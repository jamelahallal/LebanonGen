...
function Dashboard() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8081/region-status-stats")
      .then((res) => {
        const formatted = {};

        res.data.forEach((item) => {
          formatted[item.region] =
            Number(item.carriers) + Number(item.infected);
        });
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-4">Lebanon Cases Map</h1>

      {cases ? <LebanonMap cases={cases} /> : <p>Loading map...</p>}
    </div>
  );
}
