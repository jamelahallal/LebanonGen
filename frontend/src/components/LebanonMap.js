    ...
    if (viewMode === "carriers") {
      ...
      statsHTML = `
        ...
        <div style="display:flex;justify-content:space-between;font-weight:800;border-top:1px dashed rgba(255,255,255,0.2);padding-top:6px;font-size:12px;margin-top:4px;">
          <span>${t("map.total_label")}</span>
          <span style="color:${regionData.total > 0 ? "#ff8787" : "#4ade80"};">${regionData.total}</span>
        </div>`;
    }

    const tooltipContent = `
      <div style="background:rgba(15,23,42,0.98);color:white;padding:12px 16px;border-radius:8px;
        font-family:system-ui,-apple-system,sans-serif;font-size:11px;
        box-shadow:0 10px 25px -5px rgba(0,0,0,0.3);border:1px solid rgba(255,255,255,0.15);
