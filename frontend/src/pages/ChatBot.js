...
function ChatBot() {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-slate-50 p-2 md:p-4 font-sans">
      {/* Container height changed from fixed 700px to responsive viewport height logic */}
      <div className="w-full max-w-lg h-[90vh] md:h-[700px] bg-white rounded-3xl shadow-2xl flex flex-col overflow-hidden border border-gray-100">
        {/* Header */}
        <div className="bg-[#b30000] p-4 md:p-6 flex items-center justify-between text-white shadow-md">
          <div className="flex items-center gap-3 md:gap-4">
            <div className="relative p-2 bg-white/20 rounded-2xl">
              <Bot size={28} className="md:size-8" />
              <div className="absolute bottom-1 right-1 w-3 h-3 bg-green-400 border-2 border-[#b30000] rounded-full"></div>
            </div>
            <div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6 bg-white">
          {messages.map((msg, index) => (
            ...
            >
              <div
                className={`max-w-[90%] p-3 md:p-4 rounded-2xl shadow-sm ${msg.sender === "user" ? "bg-[#b30000] text-white rounded-br-none" : "bg-gray-50 text-gray-800 border border-gray-100 rounded-bl-none"}`}
              >
        ...
        <div className="p-3 md:p-4 bg-gray-50/50 border-t border-gray-100">
          ...
          <form onSubmit={handleSend} className="flex gap-2 md:gap-3">
            <input
              ...
              className="flex-1 bg-white border border-gray-200 rounded-2xl px-4 py-2 md:px-5 md:py-3 text-sm focus:ring-2 focus:ring-[#b30000]/10 focus:border-[#b30000] outline-none transition-all shadow-inner"
            />
            <button
              type="submit"
              disabled={!input.trim() || isTyping}
              className="bg-[#b30000] text-white p-3 rounded-2xl md:p-3.5 hover:bg-[#900000] transition-all shadow-lg active:scale-95 disabled:bg-gray-300"
            >
              <Send size={18} md={20} />
            </button>
          </form>
